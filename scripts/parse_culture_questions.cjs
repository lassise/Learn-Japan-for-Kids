...
for (let i = 1; i <= 20; i++) {
    ...
    for (let q = 1; q <= 30; q++) {
        ...
        const safeScenario = quest.scenario.replace(/'/g, "''").replace(/’/g, "''"); // Handle smart quotes too
        const safeQuestion = quest.question.replace(/'/g, "''").replace(/’/g, "''");

        // Fix options JSON
        // Options array of objects needs to be JSON string
        // We used letter IDs "A", "B", "C".
        const safeOptions = JSON.stringify(quest.options).replace(/'/g, "''").replace(/’/g, "''");
